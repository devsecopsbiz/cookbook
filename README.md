...
* Start by creating a new Project using a preselected [Lab](https://azuredevopsdemogenerator.azurewebsites.net/?name=WhiteSource-Bolt&templateid=77362)
    1. Navigate to the Azure DevOps Demo Generator and *Sign In* with the credentials you've used to create the Azure DevOps Organization:
        ...
        ![Azure DevOps Demo Generator - Sign In](img/AzureDevOpsLab-SignIn.png =800x)

    2. Select your Organization from the dropdown, fill in the Project Name, and *Create Project*:

        ![Azure DevOps Demo Generator - Create Project](img/AzureDevOpsLab-CreateProject-01.png =800x)

    3. Wait for a few seconds for the project to create and then *Navigate* to it:

        ![Azure DevOps Demo Generator - Create Project](img/AzureDevOpsLab-CreateProject-02.png =400x)
...
* Import additional Git repos, required for the labs:
    ...
    1. Navigate to *Repos* and on the upper repositories drop down, select *Import repository*:
        ...
        ![Azure DevOps Demo Generator - Navigate to Project](img/AzureDevOpsLab-ImportRepo.png =800x)
...
* Wait a few seconds for the import process to finish and navigate to *Pipelines*, *Builds*:
    ...
    ![Navigate to Pipelines, Build](img/AzureDevOpsLab-Builds-01.png =400x)
    ...
    1. Under *New*, select *Import a pipeline* to import a precooked pipeline to build the recently imported repo:
        ...
        ![Import a build pipeline](img/AzureDevOpsLab-Builds-02.png =800x)
    ...
    2. Drag and drop the `SmartHotel_Petchecker-Web.json` file, or Browse for the same file. Then, press *Import* to start the import process:
        ...
        ![Start the build pipeline import process](img/AzureDevOpsLab-Builds-03.png =400x)
    
    3. Select the `Hosted VS2017` Agent pool:

        ![Build pipeline agent pool](img/AzureDevOpsLab-Builds-04.png =800x)

    4. Now select the Source Repo to the recently imported repository:

        ![Build pipeline Source repo](img/AzureDevOpsLab-Builds-05.png =800x)

    5. To finish, enable the *Continuous integration* trigger, saving the build after it:

        ![Build pipeline trigger](img/AzureDevOpsLab-Builds-trigger.png =800x)
...
* Under *Pipelines*, navigate to *Releases* and press `New pipeline`
     ...
     ![Releases pipeline](img/AzureDevOpsLab-Releases-01.png =800x)
    ...
    ![Select empty process](img/AzureDevOpsLab-Releases-02.png =800x)

    2. Navigate back to `All pipelines` and `Import a release pipeline`:

    ![All pipelines](img/AzureDevOpsLab-Releases-04.png =800x)

    ![Import release pipeline](img/AzureDevOpsLab-Releases-05.png =400x)

    3. Drag and drop, or navigate, to select the `SmartHotel360_Website-Deploy.json` file:

    ![Import release pipeline](img/AzureDevOpsLab-Releases-06.png =400x)

    4. Press *OK* to start the import process. You should now have a release pipeline like this:

    ![Import release pipeline](img/AzureDevOpsLab-Releases-07.png =800x)

    5. Navigate to the Stage tasks:

    ![Import release pipeline](img/AzureDevOpsLab-Releases-08.png =600x)

    6. Select the first task and, under *Azure Subscription*, press `New`:

    ![Release pipeline CD trigger](img/AzureDevOpsLab-Releases-09.png =800x)
    ...
    ![Import release pipeline](img/AzureDevOpsLab-Releases-trigger.png =800x)
...
1. In Azure DevOps, click on Builds, select each build and click the "Queue" button on the right upper corner.
    ...
    ![Azure DevOps Build](img/AzureDevOpsLab-Builds-Run-01.png =800x)

2. Monitor the builds and wait for the build to complete

    ![Azure DevOps Build](img/AzureDevOpsLab-Builds-Run-end.png =800x)
...
3. The release will automatically start when the build is complete (be patient, this can take some time). Review the results as it is complete. 
    ...
    ![Azure DevOps Release](img/AzureDevOpsLab-Releases-Run-executing.png =800x)
...
Navigate back to *Repos*, *SmartHotel360-Website* repo, select `appsettings.Development.json` and press *Edit*
    ...
    ![Repo change to trigger CI CD](img/AzureDevOpsLab-Repo-edit.png =800x)

5. Change the *Name* property value to something you want, and *Commit* the change after it

    ![Repo change to trigger CI CD](img/AzureDevOpsLab-Repo-commit.png =800x)
...
2. Select each build pipeline, click the "Edit" button, and select the `WhiteSource Bolt` task
    ...
    ![WhiteSource Bolt task](img/AzureDevOpsLab-Builds-WhiteSourceBolt.png =800x)

3. Now navigate to *Pipelines*, *WhiteSource Bolt* on the left and explore the generated reports

    ![WhiteSource Bolt report](img/AzureDevOpsLab-Builds-WhiteSourceBolt-report.png =800x)
...
![Navigate to User Security](img/PAT-token-security.png =800x)
![Generate PAT Token](img/PAT-token-navigate.png =400x)
![Generate PAT Token](img/PAT-token-create.png =400x)
![Copy PAT Token](img/PAT-token.png =400x)
...
![Add OWASP task](img/Add-OWASP-tasks-navigate.png =800x)
![Add OWASP task](img/Add-OWASP-tasks-search.png =800x)
...
![Add OWASP task](img/Add-OWASP-tasks-Report.png =800x)
...
![Add OWASP task](img/Add-OWASP-tasks.png =800x)
